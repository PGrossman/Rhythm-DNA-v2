...
function analyzeWithEnsemble(audioPath, opts = {}) {
  ...
  const stdoutCallback = opts.stdoutCallback; // v1.0.0: stdout parsing callback
  ...
  return new Promise(async (resolve, reject) => {
    try {
      ...
      // v1.0.0: Use spawn for real-time stdout parsing
      const { spawn } = require('child_process');
      const child = spawn(venvPy, args, { env: pyEnv });
      
      let stdout = '';
      let stderr = '';
      
      // Phase guards so we emit each milestone once
      let sent25 = false; // Demucs done/skip
      let sent50 = false; // PANNs done
      let sent75 = false; // YAMNet/CLAP done
      ...
      child.stdout.on('data', (data) => {
        ...
        // 25% — Demucs finished OR Demucs explicitly skipped
        // Heuristics: lines like "demucs", "separat", "used_demucs=true/false", "mode=mix-only"
        if (!sent25 && (low.includes('used_demucs=true') || low.includes('demucs') || low.includes('separat') || low.includes('mode=mix-only') || low.includes('used_demucs=false'))) {
          if (progressCallback) progressCallback(25, 'Demucs stage');
          sent25 = true;
        }

        // 50% — PANN(s) pass complete (look for "pann" / "panns")
        if (!sent50 && (low.includes('pann') || low.includes('panns'))) {
          if (progressCallback) progressCallback(50, 'PANNs stage');
          sent50 = true;
        }

        // 75% — YAMNet or CLAP stage complete
        if (!sent75 && (low.includes('yamnet') || low.includes('clap'))) {
          if (progressCallback) progressCallback(75, 'YAMNet/CLAP stage');
          sent75 = true;
        }
      });
      ...
      child.stderr.on('data', (data) => {
        ...
        // Downgrade known non-fatal booster TypeErrors to warnings
        const NON_FATAL_PATTERNS = [
          /TypeError:\s*_combined_mean\(\)\s*takes\s*1\s*positional\s*argument\s*but\s*2\s*were\s*given/i,
          /_combined_mean_pos\(\)\s*missing\s*3\s*required\s*positional\s*arguments/i
        ];
        const isNonFatalBoosterNoise = NON_FATAL_PATTERNS.some(rx => rx.test(line));

        if (isNonFatalBoosterNoise) {
          console.log('[ENSEMBLE][WARN] Booster non-fatal:', line.trim());
          // DO NOT mark as failure; DO NOT change badge to ERROR
          return;
        }
      ...
      child.on('close', async (code) => {
        ...
        // Only mark as failed for non-zero exit code
        const err = code !== 0 ? new Error(`Process exited with code ${code}`) : null;
        // Set process error flag
        procError = !!err;
        
        // Emit final progress regardless of stderr content (booster warnings are non-fatal)
