...
# v1.0.0: Apple Silicon Acceleration - PyTorch device selection
import os  # <-- required before sys.path.append(os.path.join(...))
import sys
sys.path.append(os.path.join(os.path.dirname(__file__), '..', 'py'))
from torch_device import choose_torch_device
_ACCEL_DEVICE = choose_torch_device()
print(f"[ACCEL] PyTorch device selected: {_ACCEL_DEVICE}")
...
def _combined_mean(stats, *args, default=0.0):
    try:
        if isinstance(stats, (int, float)):
            return float(stats)
        if isinstance(stats, dict):
            vals = []
            for k in ('mean', 'panns_mean', 'yamnet_mean', 'clap_mean'):
                v = stats.get(k)
...
def _combined_pos(stats, *args, default=0.0):
    try:
        if isinstance(stats, (int, float)):
            return float(stats)
        if isinstance(stats, dict):
            vals = []
            for k in ('pos', 'panns_pos', 'yamnet_pos', 'clap_pos'):
                v = stats.get(k)
                if isinstance(v, (int, float)):
                    vals.append(float(v))
            return sum(vals) / len(vals) if vals else float(default)
...
def _boost_mix_only_orchestral(trace, already):
    ...
    # Piano thresholds relaxed for soft/ambient keys to reduce misses:
    # - mean: 0.005 → 0.0025 (50% reduction)
    # - pos: 0.015 → 0.006 (60% reduction)
    # These values work with soft_piano_recall_v1 booster for better recall
    TH = {
        "piano":   {"mean": 0.0025, "pos": 0.006},  # Relaxed for soft/ambient piano detection - reduced from 0.005/0.015
        ...
        "brass":   {"mean": 0.0070, "pos": 0.040},  # PANNs 0.0059 + YAM 0.0016 ~= 0.0075, pos ~= 0.0413
...
def _booster_mix_only_horns_specific_v1(per_model, decision_trace, current_instruments):
    ...
    TH = {
        ...
        "inst_pos_min":       0.0075,  # was 0.008 — tiny relax to match logs
...
def _combined_mean(*vals):
    """Robust variadic function to combine multiple mean values."""
    seq = []
    for v in vals:
        if v is None:
            continue
        if isinstance(v, (list, tuple)):
            seq.extend([x for x in v if x is not None])
        else:
            seq.append(v)
    if not seq:
        return 0.0
...
def _combined_mean_trace(trace, key):
    """Get combined mean from both PANNs and YAMNet for a key (legacy wrapper)."""
    try:
        pm = _get_model_stat(trace, "panns", "mean_probs", key, 0.0)
        ym = _get_model_stat(trace, "yamnet", "mean_probs", key, 0.0)
        return _combined_mean(pm, ym)
    except Exception:
        return 0.0
    ...
    try:
        ...
        if len(instruments) <= 12:  # don't overgrow lists; normal mixes stay small
            ...
            if not _has(instruments, "Bass Guitar"):
                cm = _combined_mean_trace(trace, "bass_guitar")
        ...
        if _has(instruments, MIX_ONLY_TUNE["trumpet"]["REQUIRE_SECTION"]) and not _has(instruments, "Trumpet"):
            cm_trp = _combined_mean_trace(trace, "trumpet")
...
def _apply_mix_only_core_v2(instruments, trace):
    ...
    try:
        ...
        # Check drums
        drum_mean = _combined_mean_trace(trace, "drum_kit")
        ...
        eg_mean = _combined_mean_trace(trace, "electric_guitar")
        ...
        ag_mean = _combined_mean_trace(trace, "acoustic_guitar")
        ...
        # Check bass guitar
        bg_mean = _combined_mean_trace(trace, "bass_guitar")
...
def _apply_soft_piano_recall_v1(instruments, trace):
    """Soft piano recall booster v1 - improve detection of soft/ambient piano parts."""
    try:
        # Get piano means and ratios
        p_means = _combined_mean_trace(trace, "piano")
        p_pos = _any_pos(trace, "piano")
        
        # Get brass mean to ensure it's not brass-heavy
        brass_mean = _combined_mean_trace(trace, "brass")
        
        # Get harmonic chroma features if available
        chroma_mean = trace.get("features", {}).get("chroma_mean", 0.0)
        chroma_std = trace.get("features", {}).get("chroma_std", 1.0)
        
        # Gate conditions: soft piano characteristics
        gate_1 = (p_means >= 0.0015 or p_pos > 0)  # Some piano evidence
        gate_2 = (chroma_mean >= 0.12 and chroma_std <= 0.08)  # Harmonic, stable chroma
        gate_3 = brass_mean < 0.006  # Not brass-heavy
        
        if gate_1 and gate_2 and gate_3:
            _add(instruments, "Piano")
            
            # Log the decision
            if isinstance(trace, dict):
                trace.setdefault("boosts", {})["soft_piano_recall_v1"] = {
                    "gate_1_piano_evidence": gate_1,
                    "gate_2_harmonic_stable": gate_2,
                    "gate_3_not_brass_heavy": gate_3,
                    "p_means": p_means,
                    "p_pos": p_pos,
                    "brass_mean": brass_mean,
        ...
        # Never let tuning crash the primary pipeline
        if isinstance(trace, dict):
            trace.setdefault("errors", []).append(f"soft_piano_recall_v1: {type(e).__name__}: {e}")

def _apply_brass_recall_v2(instruments, trace):
    """Brass recall booster v2 - improve detection of brass instruments (trumpet/trombone/sax/brass section)."""
    try:
        # Get model statistics
        panns_means = trace.get("per_model", {}).get("panns", {}).get("mean_probs", {})
        panns_pos = trace.get("per_model", {}).get("panns", {}).get("pos_ratio", {})
        yamnet_means = trace.get("per_model", {}).get("yamnet", {}).get("mean_probs", {})
        yamnet_pos = trace.get("per_model", {}).get("yamnet", {}).get("pos_ratio", {})
        
        # Compute combined means for brass instruments
        tpt = _combined_mean(panns_means.get('trumpet'), yamnet_means.get('trumpet'))
        tbn = _combined_mean(panns_means.get('trombone'), yamnet_means.get('trombone'))
        sax = _combined_mean(panns_means.get('saxophone'), yamnet_means.get('saxophone'))
        brs = _combined_mean(panns_means.get('brass'), yamnet_means.get('brass'))
        
        # Get presence ratios
        tpt_pos = panns_pos.get('trumpet', 0.0)
        tbn_pos = panns_pos.get('trombone', 0.0)
        sax_pos = panns_pos.get('saxophone', 0.0)
        brs_pos = panns_pos.get('brass', 0.0)
        
        # Compute spectral features if not available
        spectral_centroid_mean = trace.get("features", {}).get("spectral_centroid_mean", 0.0)
        onset_density = trace.get("features", {}).get("onset_density", 0.0)
        
        # If spectral features not available, use conservative defaults
        if spectral_centroid_mean == 0.0:
            spectral_centroid_mean = 0.25  # Conservative default for brass detection
        if onset_density == 0.0:
            onset_density = 0.15  # Conservative default for brass activity
        
        # Gates tuned for recall; require some mid–high spectral energy and activity
        centroid_ok = spectral_centroid_mean >= 0.22
        activity_ok = onset_density >= 0.12
        
        # Thresholds (recall-friendly, avoid false positives with centroid/activity gates)
        tpt_ok = (tpt >= 0.004 or tpt_pos >= 0.010)
        tbn_ok = (tbn >= 0.004 or tbn_pos >= 0.010)
        sax_ok = (sax >= 0.004 or sax_pos >= 0.010)
        brs_ok = (brs >= 0.006 or brs_pos >= 0.012)  # section a bit higher
        
        # Apply gates
        if not (centroid_ok and activity_ok):
            return
            
        # Add instruments based on detection flags
        if brs_ok and not _has(instruments, "Brass Section"):
            _add(instruments, "Brass Section")
            
        if tpt_ok and not _has(instruments, "Trumpet"):
            _add(instruments, "Trumpet")
            
        if tbn_ok and not _has(instruments, "Trombone"):
            _add(instruments, "Trombone")
            
        if sax_ok and not _has(instruments, "Saxophone"):
            _add(instruments, "Saxophone")
        
        # Log the decision
        if isinstance(trace, dict):
            trace.setdefault("boosts", {})["brass_recall_v2"] = {
                "centroid_ok": centroid_ok,
                "activity_ok": activity_ok,
                "spectral_centroid_mean": spectral_centroid_mean,
                "onset_density": onset_density,
                "trumpet": {"mean": tpt, "pos": tpt_pos, "detected": tpt_ok},
                "trombone": {"mean": tbn, "pos": tbn_pos, "detected": tbn_ok},
                "saxophone": {"mean": sax, "pos": sax_pos, "detected": sax_ok},
                "brass_section": {"mean": brs, "pos": brs_pos, "detected": brs_ok}
            }
            
    except Exception as e:
        # Never let tuning crash the primary pipeline
        if isinstance(trace, dict):
            trace.setdefault("errors", []).append(f"brass_recall_v2: {type(e).__name__}: {e}")

def _apply_drum_recall_v1(instruments, trace):
    """Drum recall booster v1 - improve detection of drum kit and percussion."""
    try:
        # Get model statistics for drum-related instruments
        panns_means = trace.get("per_model", {}).get("panns", {}).get("mean_probs", {})
        panns_pos = trace.get("per_model", {}).get("panns", {}).get("pos_ratio", {})
        yamnet_means = trace.get("per_model", {}).get("yamnet", {}).get("mean_probs", {})
        yamnet_pos = trace.get("per_model", {}).get("yamnet", {}).get("pos_ratio", {})
        
        # Compute combined means for drum instruments
        drumkit = _combined_mean(panns_means.get('drumkit'), yamnet_means.get('drumkit'))
        drums = _combined_mean(panns_means.get('drums'), yamnet_means.get('drums'))
        snare = _combined_mean(panns_means.get('snare'), yamnet_means.get('snare'))
        kick = _combined_mean(panns_means.get('kick'), yamnet_means.get('kick'))
        hihat = _combined_mean(panns_means.get('hi-hat'), yamnet_means.get('hi-hat'))
        
        # Get presence ratios
        drums_pos = panns_pos.get('drums', 0.0)
        ...
        # If spectral features not available, use conservative defaults
        if spectral_centroid_mean == 0.0:
            spectral_centroid_mean = 0.30  # Conservative default for drum detection
        if onset_density == 0.0:
            onset_density = 0.15  # Conservative default for drum activity
        
        # Gates tuned for drum recall
        onset_ok = onset_density >= 0.10
        centroid_ok = 0.15 <= spectral_centroid_mean <= 0.45
        
        # Thresholds (recall-leaning for drum detection)
        drumkit_ok = drumkit >= 0.005
        drums_ok = drums_pos >= 0.015
        snare_kick_ok = (snare >= 0.004 and kick >= 0.004)
        
        # Apply gates and detection
        if not (onset_ok and centroid_ok):
            return
            
        # Check if any drum detection criteria are met
        drum_detected = drumkit_ok or drums_ok or snare_kick_ok
        
        if drum_detected and not _has(instruments, "Drum Kit (acoustic)"):
            _add(instruments, "Drum Kit (acoustic)")
        
        # Log the decision
        if isinstance(trace, dict):
            trace.setdefault("boosts", {})["drum_recall_v1"] = {
                "onset_ok": onset_ok,
                "centroid_ok": centroid_ok,
                "onset_density": onset_density,
                "spectral_centroid_mean": spectral_centroid_mean,
                "drumkit": {"mean": drumkit, "detected": drumkit_ok},
                "drums": {"pos": drums_pos, "detected": drums_ok},
                "snare_kick": {"snare": snare, "kick": kick, "detected": snare_kick_ok},
                "final_detection": drum_detected
            }
            
    except Exception as e:
        # Never let tuning crash the primary pipeline
        if isinstance(trace, dict):
            trace.setdefault("errors", []).append(f"drum_recall_v1: {type(e).__name__}: {e}")
...
def _apply_mix_only_woodwinds_v1(instruments, trace):
    ...
    try:
        ...
        piano_combined_pos = _combined_mean_trace(trace, "piano")
        ...
        for key in WOODWIND_KEYS:
            cm = _combined_mean_trace(trace, key)
    ...
    for group_label, members in FAMILY_GROUPS.items():
        ...
        if rollup_cfg["require_context"] and group_label == "Woodwinds":
            context_ok = (
                ...
                (_combined_mean_trace(decision_trace, "strings") >= rollup_cfg.get("context_gate", 0.002)) or
                (_combined_mean_trace(decision_trace, "brass")   >= rollup_cfg.get("context_gate", 0.002))
            )
...
MIX_ONLY_TUNE = {
    ...
    "trumpet": {
        ...
        "POS_ANY_MIN": 0.0075,        # needs at least some pos evidence on any model
...
def _combined_mean(stats: dict | None) -> float:
    """
...
def _robust_family_rollup_v1(instruments, decision_trace):
    ...
    try:
        ...
        means = []
        for key in fam_keys:
            mean = _combined_mean_trace(decision_trace, key)
            means.append(mean)
...
def analyze(audio_path: str, use_demucs: bool = True, diag: bool = False) -> Dict[str, Any]:
    ...
    try:
        ...
        if not used_demucs:
            ...
            # Soft piano recall booster v1 - improve detection of soft/ambient piano parts
            _apply_soft_piano_recall_v1(current, trace)
            
            # Brass recall booster v2 - improve detection of brass instruments (trumpet/trombone/sax/brass section)
            _apply_brass_recall_v2(current, trace)
            
            # Drum recall booster v1 - improve detection of drum kit and percussion
            _apply_drum_recall_v1(current, trace)
    ...
    try:
        ...
        for child in wood_children:
            if child in current_instruments:
                ...
                child_mean = _combined_mean_trace(decision_trace, child)
    ...
    try:
        ...
        for child in WOODWIND_CHILDREN:
            cm = _combined_mean_trace(decision_trace, child)    # panns+yam mean
    ...
    # Sep-19 brass consolidation with conservative thresholds
    try:
        current_instruments = out.get("instruments", [])
        decision_trace = out.get("decision_trace", {})
        scores = decision_trace.get("per_model", {})
        
        # Conservative thresholds matching observed CLAP logs (0.17–0.21 brass peaks on "25 or 6 to 4")
        brass_triggers = []
        brass_triggers += [('trumpet', 0.04), ('trombone', 0.04), ('brass', 0.04)]
        
        # Get combined scores for brass instruments
        panns_means = scores.get("panns", {}).get("mean_probs", {})
        yamnet_means = scores.get("yamnet", {}).get("mean_probs", {})
        
        # Calculate combined scores
        combined_scores = {}
        for key in ['trumpet', 'trombone', 'brass']:
            panns_score = panns_means.get(key, 0.0)
            yamnet_score = yamnet_means.get(key, 0.0)
            combined_scores[key] = (panns_score + yamnet_score) / 2.0
        
        # If any single brass-key >= 0.04 OR two brass-family keys >= 0.03 each → add "Brass (section)"
        hits = [k for (k, thr) in brass_triggers if combined_scores.get(k, 0.0) >= thr]
        pair_hits = sum(1 for k in ['trumpet', 'trombone', 'brass'] if combined_scores.get(k, 0.0) >= 0.03)
        
        if hits or pair_hits >= 2:
            if "Brass (section)" not in current_instruments:
                current_instruments.append("Brass (section)")
        
        out["instruments"] = current_instruments
        
    except Exception as _e:
        # Never break pipeline if brass consolidation hiccups
        out.setdefault("errors", []).append(f"brass_consolidation_failed:{type(_e).__name__}:{_e}")
...
# --- Robust overrides for stat combination ---
def _combined_mean(stats, *args, default=0.0):
    try:
        if isinstance(stats, (int, float)):
            return float(stats)
