...
// Consolidate instrument sections (restore Sep-19 BRASS section tagging)
function consolidateSections(list) {
  const lower = new Set(list.map(x => x.toLowerCase()));
  
  // BRASS recall: if any explicit brass-family appears or "brass" itself, add section.
  const brassKeys = ['brass', 'trumpet', 'trombone', 'cornet', 'flugelhorn', 'horn', 'french horn'];
  const hasBrass = brassKeys.some(k => lower.has(k));
  if (hasBrass) list.push('Brass (section)');

  // Woodwinds (keep as you had it before)
  const wwKeys = ['woodwinds', 'saxophone', 'clarinet', 'flute', 'oboe', 'bassoon', 'english horn'];
  const hasWW = wwKeys.some(k => lower.has(k));
  if (hasWW && !lower.has('woodwinds (section)')) list.push('Woodwinds (section)');
...
export async function runEnsemble({ audioPath, workdir, pythonBin, demucs = 0 }) {
  ...
  return await new Promise((resolve) => {
    ...
    py.on("close", async (code) => {
      ...
      // Apply section consolidation to instruments
      const consolidatedInstruments = consolidateSections(instruments);
      
      // Build the canonical list and flags
      const finalList = finalizeInstruments({
        ensembleInstruments: consolidatedInstruments,
      ...
      const result = {
        ...
        instruments_ensemble: consolidatedInstruments,
        usedDemucs,
        byStem: norm.byStem || {},
        scores: norm.scores || {},
